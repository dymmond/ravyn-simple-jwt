...
from ravyn_simple_jwt.backends import (
...
async def test_raise_405(async_client):
    ...
    response = await async_client.get("/auth/signin")
...
async def test_raise_401(async_client):
    ...
    response = await async_client.post("/auth/signin", json=data)
...
async def test_refresh_access_token_successfully(async_client):
    ...
    response = await async_client.post(
        "/auth/refresh-access", json={"refresh_token": tokens["refresh_token"]}
    )
    assert response.status_code == 200
    assert "access_token" in response.json()
